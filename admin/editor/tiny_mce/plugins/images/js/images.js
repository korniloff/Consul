...
$(function(){
	...
	$('.imageBlock0').live('dblclick', function(){
		...
		if(e.attr('type') == 'files')
		{
			...
			var text = '<a href="'+SITE_FOLDER+e.attr('linkto')+'" '+addAttr+' title="'+e.attr('fname')+'">';
			text += e.attr('fname');
			text += '</a> ' + ' ('+filesize+') ';
		}
		else
		{
			if(e.attr('fmiddle')) {
				...
				var text = '<a href="'+SITE_FOLDER+e.attr('linkto')+'" '+addAttr+' title="'+e.attr('fname')+'">';
				text += '<img src="'+SITE_FOLDER+e.attr('fmiddle')+'" width="'+e.attr('fmiddlewidth')+'" height="'+e.attr('fmiddleheight')+'" alt="'+e.attr('fname')+'" />';
				text += '</a> ';
			} else {
				var text = '<img src="'+SITE_FOLDER+e.attr('linkto')+'" width="'+e.attr('fwidth')+'" height="'+e.attr('fheight')+'" alt="'+e.attr('fname')+'" /> ';
			}
		}
